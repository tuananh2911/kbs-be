...
export class AppController {
  constructor(private readonly dishRepository: Repository<Dish>) {}

  @Get('gym-mode')
  async getCaloConsumed(@Body('body') body: UserGymDto): Promise<any> {
    ...
    const dishBreakfast = await this.dishRepository.findBy({BuoiPhucVu: 'Buổi Sáng'});
    const mealBreakfast = fuzzyLogicNutrition.luaChonMonAn(dishBreakfast, goalCalo * 0.3);
    const dishLunch = await this.dishRepository.findBy({BuoiPhucVu: 'Buổi Trưa'});
    const mealLunch = fuzzyLogicNutrition.luaChonMonAn(dishLunch, goalCalo * 0.4);
    const dishDinner = await this.dishRepository.findBy({BuoiPhucVu: 'Buổi Tối'});
    const mealDinner = fuzzyLogicNutrition.luaChonMonAn(dishDinner, goalCalo * 0.3);
    return {caloConsumed: caloConsumed, caloNeed: goalCalo, timeToGoal: timeToGoal, mealBreakfast:mealBreakfast,mealLunch:mealLunch,mealDinner:mealDinner }
  }
}
